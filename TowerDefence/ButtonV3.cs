    ...
    public class ButtonV3
    {
        ...
        public void Update(GameTime gameTime, Screen screen)
        {
            ...
            if (MouseInput.IsActived)
            {
                ...
                /// Discretos LD
                /// IsReleased
                if (Dimension.Intersects(MouseInput.GetRectangle(screen)) && MouseInput.mouseState.LeftButton == ButtonState.Released && MouseInput.oldmouseState.LeftButton != ButtonState.Released)
                    isReleased = true;
                else
                    isReleased = false;
